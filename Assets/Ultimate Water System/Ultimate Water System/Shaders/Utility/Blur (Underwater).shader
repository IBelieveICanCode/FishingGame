...
Shader "UltimateWater/Utilities/Blur (Underwater)"
{
    Properties
    {
        _MainTex("", 2D) = "" {}
        ...
        {
            color += tex2D(_MainTex, UnityStereoTransformScreenSpaceTex(uv)) * _Weights[index];
        }

        half4 frag(VertexOutput vo) : SV_Target
        {
            half mask = 1.0 - tex2D(_UnderwaterMask, UnityStereoTransformScreenSpaceTex(vo.uv3));
            ...
            return lerp(color, tex2D(_MainTex, UnityStereoTransformScreenSpaceTex(vo.uv3)), mask);
        }
