Shader "UltimateWater/Utility/Noise"
{
    Properties
    {
    ...
    float4 fragRGB_01_fade(VertexOutput vo) : SV_Target
    {
        float4 uv = float4(vo.uv, 0, 0) + _Offset;
        float4 coord1 = uv * _Period.x;
        float4 coord2 = uv * _Period.y;
        ...
        float2  dump = 1.0f - pow(saturate(abs(vo.uv - 0.5f) * 2.0f), 2.0f);

        return lerp(0.75f, val, min(dump.x, dump.y));
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma vertex vert
            #pragma fragment fragRGB_01_fade
            //#define NOISE_FUNC Perlin3D(coord.xyz)
