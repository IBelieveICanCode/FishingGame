...
inline UnityGI UnityGlobalIllumination(UnityGIInput data, half occlusion, half oneMinusRoughness, half3 normalWorld, bool reflections, half2 dirRoughness)
{
    ...
    #if defined(_CUBEMAP_REFLECTIONS) && !defined(DEFERRED)
    if(reflections)
    {
    ...
    //half waterSurfaceDepth = globalWaterData.depth;
    //half sceneDepth = waterSurfaceDepth - LinearEyeDepthHalf(SAMPLE_DEPTH_TEXTURE_PROJ(_WaterlessDepthTexture, globalWaterData.refractedScreenPos).r);
    ...
    o_gi.indirect.diffuse = subsurfaceScattering.rgb * subsurfaceScattering.a * (1.0 + (1.0 - globalWaterData.totalMask.w) * _SubsurfaceScatteringShoreColor);
    #else
