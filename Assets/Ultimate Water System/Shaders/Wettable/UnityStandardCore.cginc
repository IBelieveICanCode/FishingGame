...
inline FragmentCommonData MetallicSetup (float4 i_tex)
{
	...
	// WETTABLE START
		...
		half2 wetness;
		wetness.xy = tex2D(_WetnessMap, i_tex.zw / _DetailAlbedoMap_ST.xy).xx * half2(0.08, 0.01);
		wetness.xy = saturate(wetness.xy) * _WetEffectIntensity;

		// wetness
		metallic = lerp(metallic, 1.0, wetness.y);
		...
		// wetness
		diffColor *= 1.0 - wetness.x * 0.3;
		//oneMinusRoughness = lerp(oneMinusRoughness, 0.86, wetness.x);
		oneMinusRoughness += (1.0 - oneMinusRoughness) * wetness.x * 0.4;
		oneMinusReflectivity = lerp(oneMinusReflectivity, 1.0, wetness.x);
